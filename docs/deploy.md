...
- **Roll deployment** - the deploy job assembles kubeconfig from the standard `K8S_SERVER`, `K8S_CA_DATA`, `K8S_CLIENT_CERT_DATA`, and `K8S_CLIENT_KEY_DATA` Forgejo secrets, then points kubectl at the k3s API on the LAN IP `192.168.0.194:6443` (in the cert SANs) as the deployer ServiceAccount. The app pod is pinned to kai-server, where `registries.yaml` has the insecure-registry entry, so the cluster-side pull of `192.168.0.194:30500/...` lands on that node. See infrastructure#171.
